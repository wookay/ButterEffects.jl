...
function show_effectbits_suffix(io::IO, mime::MIME"text/plain", suffix::Union{Char, String})
    doc = if suffix == 'c'
        """
- `consistent::UInt8`:
...
end # function show_effectbits_suffix

function show_effectbits_letter(io::IO, mime::MIME"text/plain", letter::EffectBitsLetter)
    print(io, "### Effects: ")
    Base.show(io, mime, letter)
    println(io)
    show_effectbits_suffix(io, mime, letter.suffix)
end # function show_effectbits_letter
